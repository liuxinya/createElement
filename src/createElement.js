import {isArray, isNumber, isObject, isString} from './common.js';
...
export function createElement(jsxNode) {
    const tagName = jsxNode.elementName;
    const props = jsxNode.attributes || {};
    const childNodes = jsxNode.children || [];
    const tag = HTML_TAGS[tagName];
    const object = isObject(tag);
    ...
    childNodes.forEach((childNode) => {
        ...
        if (isObject(childNode)) {
            ...
            // 有种特殊情况 childNode还是数组
            childNode.forEach((item) => {
                el.appendChild(createElement(item));
            });
        } else if (isString(childNode) || isNumber(childNode)) {
            el.appendChild(document.createTextNode(childNode));
        } else {
            throw new Error(
                `Expected 'object' or 'string' but received '${typeof value}'`
            );
        }
    });
    return el;
